    ...
    def validate_field_many2many(self, field_name, field_ttype, field_value,
        ...
        if field_required and field_value in (None, ""):
            ...
            name_relational_model = self.env[
                'account.analytic.line'].fields_get()[field_name]['relation']
        ...
        if self and self.file:
            ...
            if self.import_type == 'csv':
                for rec in self:
                    ...
                    try:
                        file = str(
                            base64.decodebytes(rec.file).decode('utf-8'))
                        ...
                        for row in myreader:
                            try:
                                ...
                                if row[0] not in (None, "") and row[1] not in (
                                        None, "") and row[2] not in (
                                            None, "") and row[3] not in (None,
                                                                         ""):
                                    vals = {}
                                    ...
                                    if search_project and search_project.allow_timesheets:
                                        ...
                                        skipped_line_no[str(
                                            counter
                                        )] = " - Project not found or it's not Allow timesheets"
                                        counter = counter + 1
                                        continue

                                    if row[4] not in (None, ""):
                                        search_task = self.env[
                                            'project.task'].search(
                                                [('name', '=', row[4])],
                                                limit=1)
                                        if search_task and search_task.project_id == search_project:
                                            ...
                                            skipped_line_no[str(
                                                counter
                                            )] = " - Task not found or it's not belongs to this project - " + search_project.name
                                            counter = counter + 1
                                            continue
                                    ...
                                    skipped_line_no[str(
                                        counter
                                    )] = " - Date, Employee, Description or Project field is empty. "
                                    counter = counter + 1

                            except Exception as e:
                                skipped_line_no[str(
                                    counter
                                )] = " - Value is not valid. " + ustr(e)
                                counter = counter + 1
                                continue
                    ...
                    if counter > 1:
                        completed_records = (counter -
                                             len(skipped_line_no)) - 2
                        res = rec.show_success_msg(completed_records,
                                                   skipped_line_no)
                        return res
                        ...
                        for row in range(sheet.nrows):
                            try:
                                ...
                                if sheet.cell(row, 0).value not in (
                                        None, ""
                                ) and sheet.cell(row, 1).value not in (
                                        None, "") and sheet.cell(
                                            row, 2).value not in (
                                                None, "") and sheet.cell(
                                                    row,
