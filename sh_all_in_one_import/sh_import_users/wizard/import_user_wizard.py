...
class ImportUserWizard(models.TransientModel):
    ...
    group_import_type = fields.Selection([('id', 'Id'), ('name', 'Name')],
                                         default='id')
        ...
        if self and self.file:
            ...
            if self.import_type in ['csv', 'excel']:
                    ...
                    for row in values:
                        try:
                            ...
                            if not user_id:
                                        ...
                                        if ',' in group:
                                            ...
                                            for group_str in comma_split_str:
                                                ...
                                                for rec in group_split_str:
                                                    group_category_id = self.env[
                                                        'ir.module.category'].sudo(
                                                        ).search([
                                                            ('name', '=', rec)
                                                        ],
                                                                 limit=1)
                                ...
                                if row[0] != '':
                                    if row[0]:
                                        user_id.sudo().write({'name': row[0]})
                                if row[1] != '':
                                    if row[1]:
                                        user_id.sudo().write({'login': row[1]})
                                if row[2] != '':
                                    if row[2]:
                                        user_id.sudo().write({
                                            'password':
                ...
                if counter > 1:
                    ...
                    res = self.show_success_msg(completed_users,
                                                skipped_line_no)
                    return res
