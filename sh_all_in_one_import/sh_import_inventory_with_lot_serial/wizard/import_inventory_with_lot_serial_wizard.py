                        ...
                        try:
                            ...
                            if row[0] not in (None, ""):
                                ...
                                if search_product and search_product.type == 'product' and search_product.tracking in ['serial', 'lot']:
                                    quant_id = self.env['stock.quant'].search([('location_id','=',self.location_id.id),('company_id','=',self.env.company.id),('product_id','=',search_product.id)],limit=1)
                                    quant_vals = {}
                                    if quant_id:
                                        quant_vals.update({
                                        ...
                                        if row[1] not in (None, ""):
                                            quant_vals.update({'inventory_quantity': row[1]})
                                        else:
                                            quant_vals.update({'inventory_quantity': 0.0})
                                        ...
                                        if row[3] not in (None, ""):
                                            ...
                                            if search_lot:
                                                ...
                                                counter = counter + 1
                                                continue
                                    quant_id = self.env['stock.quant'].sudo().create(quant_vals)
                            ...
                            if sheet.cell(row, 0).value not in (None, ""):
                                ...
                                if search_product and search_product.type == 'product' and search_product.tracking in ['serial', 'lot']:
                                    ...
                                    if quant_id:
                                        quant_vals.update({
                                            'display_name':self.name,
                                            'inventory_quantity':sheet.cell(row, 1).value,
                                            'product_id':search_product.id,
                                            'location_id':self.location_id.id
                                        })
                                        ...
                                        if sheet.cell(row, 1).value not in (None, ""):
                                            quant_vals.update({'inventory_quantity': sheet.cell(row, 1).value})
                                        else:
                                            quant_vals.update({'inventory_quantity': 0.0})
