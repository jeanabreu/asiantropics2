                        ...
                        try:
                            ...
                            if row[0] != '':
                                ...
                                active_inv = self.env['account.move'].search(
                                    [('id', '=',
                                      self.env.context.get('sh_inv_id'))],
                                    limit=1)
                                if search_product and active_inv:
                                        ...
                                        if active_inv and active_inv.partner_id:
                                            if active_inv.partner_id.lang:
                                                product = search_product.with_context(
                                                    lang=active_inv.partner_id.
                                                    lang)
                                            else:
                                                product = search_product

                                            name = product.partner_ref
                                        if active_inv.move_type in (
                                                'in_invoice',
                                                'in_refund') and product:
                                    ...
                                    account = False
                                    if active_inv.move_type in ('out_invoice',
                                                                'out_refund'):
                                    ...
                                    if row[3] in (None, ""):
                                        ...
                                        ) and search_product.uom_po_id:
                                            vals.update({
                                                'product_uom_id':
                                                search_product.uom_po_id.id
                                            })
                                        elif search_product.uom_id:
                                            vals.update({
                                                'product_uom_id':
                                                search_product.uom_id.id
                                            })
                                    else:
                                        search_uom = self.env[
                                            'uom.uom'].search(
                                    ...
                                    if row[4] in (None, ""):
                                        if active_inv.move_type in (
                                            ...
                                            vals.update({
                                                'price_unit':
                                                search_product.standard_price
                                            })
                                        else:
                                            vals.update({
                                                'price_unit':
                                                search_product.lst_price
                                            })
                                    else:
                                    ...
                                    if row[5].strip() in (None, ""):
                                        if active_inv.move_type in (
                                                'in_invoice', 'in_refund'
                                        ) and search_product.supplier_taxes_id:
                                            vals.update({
                                                'tax_ids':
                                                [(6, 0, search_product.
                                                  supplier_taxes_id.ids)]
                                            })
                                        elif active_inv.move_type in (
                                                'out_invoice', 'out_refund'
                                        ) and search_product.taxes_id:
                                            vals.update({
                                                'tax_ids':
                                                [(6, 0,
                                                  search_product.taxes_id.ids)]
                                            })
