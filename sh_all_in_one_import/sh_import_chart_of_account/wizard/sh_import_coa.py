                    ...
                    for row in myreader:
                        try:
                            ...
                            if row[2] not in ("",None):
                                type_id = self.env['account.account.type'].sudo().search([('name','=',row[2])],limit=1)
                                if type_id:
                                    vals.update({
                            ...
                            if row[5] not in ("",None):
                                group_id = self.env['account.group'].sudo().search([('name','=',row[5])],limit=1)
                                if group_id:
                                    vals.update(
                        ...
                        try:
                            ...
                            if sheet.cell(row, 0).value not in ("",None):
                                if isinstance(sheet.cell(row, 0).value, float):
                                    ...
                                    domain = [('code', '=', code)]
                                    already_account = self.env['account.account'].search(domain)
                                    if already_account:
                                        skipped_line_no[str(
                            ...
                            if sheet.cell(row, 2).value not in ("",None):
                                type_id = self.env['account.account.type'].sudo().search([('name','=',sheet.cell(row, 2).value)],limit=1)
                                if type_id:
                                    vals.update({
                                        'user_type_id':type_id.id
                                        })
                            ...
                            if sheet.cell(row, 5).value not in ("",None):
                                group_id = self.env['account.group'].sudo().search([('name','=',sheet.cell(row, 5).value)],limit=1)
                                if group_id:
                                    vals.update(
                                        {'group_id': group_id.id})
                            ...
                            if sheet.cell(row, 7).value not in ("",None):
                                ...
                                elif sheet.cell(row, 7).value.strip() == 'TRUE':
                                    vals.update({
                                        'deprecated':True
                                        })
                            is_any_error_in_dynamic_field = False
