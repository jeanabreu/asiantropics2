...
class ImportLotSerialWizard(models.TransientModel):
    ...
    lot_type = fields.Selection([('lot', 'Lot'), ('serial', 'Serial')],
                                default='lot',
                                string='Lot/Serial Type',
                                required=True)
    is_create_lot = fields.Boolean("Create Lot/Serial ?")
    display_is_create_lot = fields.Boolean('Display Is Create Lot/Serial',
                                           default=False)
    ...
    def default_get(self, fields_list):
        ...
        if move_id.picking_id.picking_type_id.code == 'incoming':
            res.update({'display_is_create_lot': True})
        return res
                        ...
                        try:
                            ...
                            if row[0] != '':
                                        ...
                                        if move_id.picking_id.picking_type_id.code == 'incoming':
                                            ...
                                            if row[1] != '':
                                                if int(row[1]) == 1:
                                                    ...
                                                    skipped_line_no[str(
                                                        counter
                                                    )] = " Quantity must be equal to one for import serial numbers. "
                                                    counter = counter + 1
                                                    continue
                                            else:
                                                vals.update({
                                ...
                                vals.update({
                                    ...
                                    'picking_id':
                                    move_id.picking_id.id,
                                    'move_id':
                                    move_id.id,
                                    'product_uom_id':
                                    move_id.product_uom.id,
                                    'location_id':
                                    move_id.location_id.id,
                                    'location_dest_id':
                                    move_id.location_dest_id.id,
                                })
