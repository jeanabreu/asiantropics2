...
class ImportJournalItemWizard(models.TransientModel):
    ...
    def import_journal_item_apply(self):
        ...
        #perform import lead
        if self and self.file and self.env.context.get('sh_journal_entry_id',
                                                       False):
            #For CSV
                        ...
                        try:
                            ...
                            if row[0] not in (None, ""):
                                ...
                                if row[3] not in (None, ""):
                                    search_analytic_acccount = self.env[
                                        'account.analytic.account'].search(
                                            [('name', '=', row[3])], limit=1)
                                    if search_analytic_acccount:
                                        vals.update({
                                            'analytic_account_id':
                                            search_analytic_acccount.id
                                        })
                                    else:
                                        skipped_line_no[str(
                                            counter
