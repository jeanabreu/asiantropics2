...
class ReporteBanco(models.AbstractModel):
    ...
    def lineas(self, datos):
        lineas = []
        for linea in self.env['account.move.line'].search(
                [('account_id', '=', datos['cuenta_bancaria_id'][0]), ('date', '>=', datos['fecha_desde']),
                 ('date', '<=', datos['fecha_hasta'])], order='date'):
            detalle = {
                'fecha': linea.date,
    ...
    def balance_inicial(self, datos):
        self.env.cr.execute(
            'select coalesce(sum(debit) - sum(credit), 0) as balance, coalesce(sum(amount_currency), 0) as balance_moneda from account_move_line where account_id = %s and date < %s',
